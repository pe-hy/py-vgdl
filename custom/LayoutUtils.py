...
def create_string_with_atoms(walkable, grid_size, atoms, obstacles,
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            flag = False
            for k, v in atom_obstacles_dict.items():
                if (i, j) == k:
                    string += v
                    flag = True
            if flag:
...
def get_room_border(graph, room):
    y, x, h, w = room
    y, x = toG(y, x)
    surrounding_nodes = [(y - 1 + i, x - 1 + j) for i in range(h + 2) for j in range(w + 2) if
                         i in [0, h + 1] or j in [0, w + 1]]
    return get_connecting_edges(surrounding_nodes)
...
def customIntersection(setA, setB):
    intersection = setA.intersection(set(setB).union(set(get_path_border(setB))))
    if len(intersection) > 0:
        return intersection
    return set()
...
def get_rooms_graph(chosen_rooms, paths_lst, rooms_points):
    ...
    for corridor_key, corridor_points in paths_lst:
        ...
        room_G.add_edge(start_room_index, end_room_index, corridor_key=edge_id, crossed_corridors=crossed_corridors,
...
def get_edges_from_skels(crossing_edges, groups_to_connect, skels, vertices_edges_dict_inv, room_G, paths_lst,
    ...
    i, j = groups_to_connect
    group_points = [get_points_from_group(i, skels, vertices_edges_dict_inv, room_G, paths_lst) for i, _ in
                    enumerate(skels)]
    for c_e in crossing_edges:
        ...
        edge_points = edge_points_only.difference((set(rooms_points).union(set(get_path_border(rooms_points)))))
        ...
        for i, edge in enumerate(crossed_edges):
            ...
            if edge == None:
                skip = True
                break
            crossed_edges_points = paths_lst[edge][1]
            if len(set(crossed_edges_points).intersection(edge_points)) != 0:
                skip = True
                break
